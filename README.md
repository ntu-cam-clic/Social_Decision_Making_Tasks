...
* **Images_PlayerB_Version/Social Value Orientation** folder. This folder stores images for the Player B version of the social value orientation task used in QualtricsCode_socialDMtasks_template_SVO(Player_B_version).qsf.
* **example_Code_Data_Preprocessing** folder. This folder contains two example raw datasets from Qualtrics and scripts for preprocessing the raw data and generating scores for each task.
...
* **QualGames_tasks_Singapore_Dataset.qsf**. This is a “Qualtrics Survey Format” file, which contains the survey questions, Javascript, survey header, CSS, embedded data for the 12 decision-making tasks used to collect the Singapore dataset and can be transferred to another Qualtrics account. Please note that if you import a .qsf file into your own account, the images in the original account will not be transferred. If the images in the original account are deleted, you will lose access to them. The images used in this QualGames_tasks_Singapore_Dataset.qsf file are stored in the above "Images" folder. You can save these images to your own GitHub account and change the URL links in the header of the survey using the "updatingImageURLs.py" script below.
* **QualGames_tasks_US_Dataset.qsf**. This is a .qsf file contains the survey questions, Javascript, survey header, CSS, embedded data for the 10 decision-making tasks used to collect the U.S. dataset and can be transferred to another Qualtrics account. The images used in this .qsf file are stored in the above "Images_supplementaryTasks" folder.
* **QualGames_allTasks.qsf**. Users are suggested to use this file to deploy the tasks. It is a combined version of the two .qsf files described above and contains all tasks used to collect the Singapore and U.S. datasets. Each task is contained within a single Qualtrics block, allowing for flexible and modular implementation. Users can simply delete tasks they do not need and can use Qualtrics branching and randomization functions to customize task order.
Note that some tasks were originally designed to appear in a fixed sequence to facilitate participant understanding. Users may choose to follow the original order or modify the instructions and task sequence to suit their own study design. Unlike the two .qsf files described above, task-specific labels such as “Game 4 Instructions” have been replaced with the generic label “Game Instructions” for each task. In addition, some tasks that spanned multiple blocks in the original .qsf files have been consolidated into a single block.
* **QualGames_tasks_template_SVO(Player_B_version).qsf**. This .qsf file contains a Player B version of the social value orientation task, in which a participant in the role of Player B receives points based on choice data from previous participants (Player A, who allocated the points). The choice data must be collected in advance and uploaded to the Qualtrics library (see below for detailed instructions). In the original version of the task, Player B is not a real participant. If researchers wish for Player B to receive points and be compensated accordingly, this file can be used as a template. The images used in this .qsf file are stored in the “Images_PlayerB_Version/Social Value Orientation” folder.
