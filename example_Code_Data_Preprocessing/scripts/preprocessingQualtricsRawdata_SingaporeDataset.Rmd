...
# This script accompanies the "QualGames_tasks_Singapore_Dataset.qsf" file for decision-making tasks and serves as an example script to process 
# raw data exported from Qualtrics. You can find a small raw data file named "exampleRawDataFromQualtrics_Singapore_Dataset.csv" in the same folder as this script. 
...
# created by Shengchuang Feng, Mar, 2024; updated June, 2026
...
rawDataPath = 'rawData/';
preprocessedDataPath = 'preprocessedData_Singapore_Dataset/';

setwd(paste0(myPath,rawDataPath));  # set the working directory
rawdata = read.csv('exampleRawDataFromQualtrics_Singapore_Dataset.csv',check.names=FALSE);  # load the data from one Qualtrics account,check.names=FALSE to keep space in names
...
for (t in c(loopStart:loopEnd)) {  # t is for the order of a questionnaire.
  ...
  #Now you have matrices for each task (subjects by rounds); you can do further calculations you want
  ###%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  for (sub in c(1:numsub)) { 
      ...
      if (unlist(questionnaireNames[t])[1]=="SVO"){ 
        ...
        SVO=atan((meanOther-50)/(meanSelf-50));    # transitivity is tested in the following section
  ...
  #======================================== PD, SH, BS, TGnh =================================================  
  else if (sum(str_detect(nameStrings,'PD_')) || sum(str_detect(nameStrings,'SH_')) || sum(str_detect(nameStrings,'BS_')) || sum(str_detect(nameStrings,'TGnh_'))  ){
  ...
  #======================================== TGb ================================================= 
    ...
    response_mat_names=c(paste0(unlist(Responses_roundLableList[t]),''),
                         paste0(unlist(TimingClick_roundLableList[t]),'_RT'),  # response time
                         paste0(unlist(TimingClick_roundLableList[t]),'_SelfOutcome'), # outcome for other 
                         paste0(unlist(TimingClick_roundLableList[t]),'_OtherOutcome'));  # outcome for self
  #======================================== RD, TG =================================================   
  } else if (sum(str_detect(nameStrings,'RD_')) || sum(str_detect(nameStrings,'TG_'))){
    oneQuestionnaire_scores=Responses_oneQuestionnaire_scoresTASK_allSub;
