...
* **Changing feedback**
  ...
  There are three ways:<br>
      1. A single set of pseudo-randomized outcomes (e.g., we can use 0 to represent that the 8 points are shown, and 1 to represent that 15 points are shown. This vector [0,0,0,1,0,1,0,1,1] can be used for all participants to hardcode the feedback. Choosing the risky option in a certain round will always give the same outcome),<br>
      2. Multiple sets of pseudo-randomized outcomes (e.g., we can have multiple vectors and one of them will be chosen for a particular participant), and<br>
