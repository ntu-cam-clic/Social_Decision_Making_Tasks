...
* **QualtricsCode_socialDMtasks_template_SVO(Player_B_version).qsf**. This is a .qsf file contains a Player B version of the social value orientation task, used to allocate points to Player B. In the original version the social value orientation task, Player B is not a real participant. If you want Player B to receive points and get paid, this file can be used as a template. The images used in this .qsf file are stored in the above "Images_supplementaryTasks" folder.
